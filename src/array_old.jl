mutable struct ROCArray{T, N, M} <: AbstractGPUArray{T, N}
    buf::DataRef{Managed{M}}
    dims::Dims{N}
    offset::Int # Offset is in number of elements (not bytes).

    function ROCArray{T, N, M}(::UndefInitializer, dims::Dims{N}) where {T, N, M <: Mem.AbstractAMDBuffer}
        @assert isbitstype(T) "ROCArray only supports bits types"
        sz::Int64 = prod(dims) * sizeof(T)
        ref = GPUArrays.cached_alloc((ROCArray, AMDGPU.device(), M, sz)) do
            @debug "Allocate `T=$T`, `dims=$dims`: $(Base.format_bytes(sz))"
            DataRef(pool_free, pool_alloc(M, sz))
        end
        return finalizer(unsafe_free!, new{T, N, M}(ref, dims, 0))
    end

    function ROCArray{T, N}(buf::DataRef{Managed{M}}, dims::Dims{N}; offset::Integer = 0) where {T, N, M <: Mem.AbstractAMDBuffer}
        @assert isbitstype(T) "ROCArray only supports bits types"
        xs = new{T, N, M}(buf, dims, offset)
...
buftype(::Type{<:ROCArray{<:Any, <:Any, M}}) where M = M # TODO check `@isdefined`?

memory_type(x::ROCArray) = memory_type(typeof(x))
memory_type(::Type{<:ROCArray{<:Any,<:Any,M}}) where {M} = @isdefined(M) ? M : Any

is_device(a::ROCArray) = memory_type(a) == Mem.DeviceMemory
is_unified(a::ROCArray) = memory_type(a) == Mem.UnifiedMemory
is_host(a::ROCArray) = memory_type(a) == Mem.HostMemory
...
function ROCArray{T,N}(::UndefInitializer, dims::Dims{N}) where {T,N}
    ROCArray{T, N, Mem.DeviceMemory}(undef, dims)
end
...
Base.similar(::ROCArray{T, <:Any, M}, dims::Base.Dims{N}) where {T, N, M} =
    ROCArray{T, N, M}(undef, dims)
Base.similar(::ROCArray{<:Any, <:Any, M}, ::Type{T}, dims::Base.Dims{N}) where {T, N, M} =
    ROCArray{T, N, M}(undef, dims)
...
function ROCArray{T,N,M}(x::AbstractArray{<:Any,N}) where {T,N,M}
    r = ROCArray{T,N,M}(undef, size(x))
    copyto!(r, convert(Array{T}, x))
    return r
end

ROCArray{T,N}(x::AbstractArray{<:Any,N}) where {T,N} = ROCArray{T,N,Mem.DeviceMemory}(x)
...
function Base.unsafe_wrap(
    ...
    M = if memtype == HIP.hipMemoryTypeUnregistered
        Mem.HostMemory
...
x = ROCArray{Float32, 1, AMDGPU.Mem.UnifiedMemory}(undef, 100)
x .= 1.0f0
...
```
"""
function Base.unsafe_wrap(::Type{Array}, A::ROCArray{T,N,M}) where {T,N,M}
    if M === Mem.UnifiedMemory || M === Mem.HostMemory
        ...
        throw(ArgumentError(
            ...
            "UnifiedMemory or HostMemory, got $M"))
    end
end
...
function Base.resize!(A::ROCVector{T}, n::Integer) where T
    ...
    #   2. Check that it is not HostMemory.
    ...
    # Preserve the buffer type (DeviceMemory, UnifiedMemory, or HostMemory)
    old_buf = convert(Mem.AbstractAMDBuffer, A.buf[])
    new_buf = if old_buf isa Mem.UnifiedMemory
        Mem.UnifiedMemory(bufsize; stream=stream())
    else
        Mem.DeviceMemory(bufsize; stream=stream())
    end
...
function Base.convert(
    ...
    ptr = convert(Ptr{T}, typeof(buf) <: Mem.HostMemory ?
        buf.dev_ptr : buf)
...
function Base.getindex(x::ROCArray{T, <:Any, <:Union{Mem.HostMemory, Mem.UnifiedMemory}}, I::Int) where T
    @boundscheck checkbounds(x, I)
    unsafe_load(pointer(x, I))
end

function Base.setindex!(x::ROCArray{T, <:Any, <:Union{Mem.HostMemory, Mem.UnifiedMemory}}, v, I::Int) where T
    @boundscheck checkbounds(x, I)
