...
using Printf: @printf
export ROCDeviceArray, ROCDeviceVector, ROCDeviceMatrix, ldg


## construction

"""
    ROCDeviceArray{T,N,A}(ptr, dims, [maxsize])

Construct an `N`-dimensional dense ROCm device array with element type `T` wrapping a
...
struct ROCDeviceArray{T,N,A} <: DenseArray{T,N}
    ...
    ROCDeviceArray{T,N,A}(ptr::LLVMPtr{T,A}, dims::Tuple,
...
Base.size(g::ROCDeviceArray{<:Any,1}) = (g.len,)
Base.length(g::ROCDeviceArray) = g.len

Base.pointer(x::ROCDeviceArray{T,<:Any,A}) where {T,A} = Base.unsafe_convert(LLVMPtr{T,A}, x)
@inline function Base.pointer(x::ROCDeviceArray{T,<:Any,A}, i::Integer) where {T,A}
    Base.unsafe_convert(LLVMPtr{T,A}, x) + Base._memory_offset(x, i)
end

typetagdata(a::ROCDeviceArray{<:Any,<:Any,A}, i=1) where {A} =
  reinterpret(LLVMPtr{UInt8,A}, a.ptr + a.maxsize) + i - one(i)


## conversions

Base.unsafe_convert(::Type{LLVMPtr{T,A}}, x::ROCDeviceArray{T,<:Any,A}) where {T,A} =
  x.ptr


## indexing intrinsics

# TODO: arrays as allocated by the HIP APIs are 256-byte aligned. we should keep track of
...
@device_function @inline function arrayref(A::ROCDeviceArray{T}, index::Integer) where {T}
...
@inline function arrayref_bits(A::ROCDeviceArray{T}, index::Integer) where {T}
    align = alignment(A)
    unsafe_load(pointer(A), index, Val(align))
end

@inline @generated function arrayref_union(A::ROCDeviceArray{T,<:Any,AS}, index::Integer) where {T,AS}
...
@device_function @inline function arrayset(A::ROCDeviceArray{T}, x::T, index::Integer) where {T}
...
@inline function arrayset_bits(A::ROCDeviceArray{T}, x::T, index::Integer) where {T}
    align = alignment(A)
    unsafe_store!(pointer(A), x, index, Val(align))
end

@inline @generated function arrayset_union(A::ROCDeviceArray{T,<:Any,AS}, x::T, index::Integer) where {T,AS}
    typs = Base.uniontypes(T)
...
@device_function @inline function const_arrayref(A::ROCDeviceArray{T}, index::Integer) where {T}
...
Base.@propagate_inbounds Base.getindex(A::ROCDeviceArray{T}, i1::Integer) where {T} =
    arrayref(A, i1)
Base.@propagate_inbounds Base.setindex!(A::ROCDeviceArray{T}, x, i1::Integer) where {T} =
...
Base.@propagate_inbounds Base.getindex(A::ROCDeviceArray,
                                       I::Union{Integer, CartesianIndex}...) =
    A[Base._to_linear_index(A, to_indices(A, I)...)]
Base.@propagate_inbounds Base.setindex!(A::ROCDeviceArray, x,
                                        I::Union{Integer, CartesianIndex}...) =
...
"""
    Const(A::ROCDeviceArray)

Mark a ROCDeviceArray as constant/read-only. The invariant guaranteed is that you will not
modify a ROCDeviceArray for the duration of the current kernel.

!!! warning
    Experimental API. Subject to change without deprecation.
"""
struct Const{T,N,AS} <: DenseArray{T,N}
    a::ROCDeviceArray{T,N,AS}
end
Base.Experimental.Const(A::ROCDeviceArray) = Const(A)
...
# deprecated
Base.@propagate_inbounds ldg(A::ROCDeviceArray, i1::Integer) = const_arrayref(A, i1)
...
function Base.reinterpret(::Type{T}, a::ROCDeviceArray{S,N,A}) where {T,S,N,A}
  err = GPUArrays._reinterpret_exception(T, a)
  err === nothing || throw(err)

  if aligned_sizeof(T) == aligned_sizeof(S) # fast case
    return ROCDeviceArray{T,N,A}(reinterpret(LLVMPtr{T,A}, a.ptr), size(a), a.maxsize)
  end

  isize = size(a)
  size1 = div(isize[1]*aligned_sizeof(S), aligned_sizeof(T))
  osize = tuple(size1, Base.tail(isize)...)
  return ROCDeviceArray{T,N,A}(reinterpret(LLVMPtr{T,A}, a.ptr), osize, a.maxsize)
end
...
@inline function _derived_array(a::ROCDeviceArray{<:Any,<:Any,A}, ::Type{T},
                                osize::Dims{N}) where {T, N, A}
  return ROCDeviceArray{T,N,A}(a.ptr, osize, a.maxsize)
end
