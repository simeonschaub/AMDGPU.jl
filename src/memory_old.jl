...
function Base.convert(::Type{Ptr{T}}, managed::Managed{M}) where {T, M}
    ...
    # if M == Mem.DeviceMemory && managed.mem.ctx != tls.ctx
    ...
    if M <: Mem.DeviceMemory
        convert(Ptr{T}, managed.mem)
    elseif M <: Mem.UnifiedMemory
        # Unified memory: accessible from device
        convert(Ptr{T}, managed.mem)
    elseif M <: Mem.HostMemory
...
# TODO workaround until we have ROCPtr
