...
function memory_stats(dev::HIPDevice = device())
    return get!(_memory_stats, dev) do
        MemoryStats()
    end
end
...
function maybe_collect(will_block::Bool = false)
    enabled = get!(_early_gc) do
        parse(Bool, get(ENV, "JULIA_AMDGPU_GC_EARLY", "true"))
    end
    enabled || return
    stats = memory_stats()
    current_time = time()
...
function memory_limits()
    return @memoize begin
        soft = if haskey(ENV, "JULIA_AMDGPU_SOFT_MEMORY_LIMIT")
            parse_limit(ENV["JULIA_AMDGPU_SOFT_MEMORY_LIMIT"])
        else
            UInt(0)
        end

        hard = if haskey(ENV, "JULIA_AMDGPU_HARD_MEMORY_LIMIT")
            parse_limit(ENV["JULIA_AMDGPU_HARD_MEMORY_LIMIT"])
        else
            UInt(0)
        end
...
function memory_limit_exceeded(bytes::Integer)
    ...
    dev = device()
    used_bytes = if stream_ordered(dev) && driver_version() >= v"6.0"
        # we configured the memory pool to do this for us
        return false
    elseif stream_ordered(dev)
        pool = pool_create(dev)
        Int(attribute(UInt64, pool, HIP.hipMemPoolAttrReservedMemCurrent))
    else
...
# TODO: extract this into a @device_memoize macro, or teach @memoize about HIPDevice?
...
const _memory_pools = PerDevice{HIP.HIPMemoryPool}()
function pool_create(dev::HIPDevice)
    return get!(_memory_pools, dev) do
        limits = memory_limits()

        # create a custom memory pool and assign it to the device
        # so that other libraries and applications will use it.
        pool = if limits.hard > 0 && AMDGPU.runtime_version() >= v"6.0"
            HIP.HIPMemoryPool(dev; maxSize = limits.hard)
        else
            HIP.HIPMemoryPool(dev)
        end
        HIP.memory_pool!(dev, pool)

        # allow the pool to use up all memory of this device
        HIP.attribute!(
            pool, HIP.hipMemPoolAttrReleaseThreshold,
...
function pool_mark!(dev::HIPDevice, val)
...
struct MemoryInfo
    ...
    function MemoryInfo()
        ...
        return new(free_bytes, total_bytes, pool_reserved_bytes, pool_used_bytes)
    end
end
...
function pool_status(io::IO = stdout, info::MemoryInfo = MemoryInfo())
    ...
    @printf(
        io, "Effective GPU memory usage: %.2f%% (%s/%s)\n",
    ...
    if info.pool_reserved_bytes === nothing
        ...
        @printf(
            io, "Memory pool usage: %s (%s reserved)\n",
    ...
    limits = memory_limits()
    return if limits.soft > 0 || limits.hard > 0
...
At each try, more and more memory is freed from the AMDGPU memory pool. When that is not
possible anymore, the latest returned value will be returned.

This function is intended for use with HIP APIs, which sometimes allocate (outside of the
AMDGPU memory pool) and return a specific error code when failing to. It is similar to
...
mutable struct Managed{M}
    ...
    function Managed(
            mem::AbstractMemory; stream = AMDGPU.stream(), synchronizing = true,
...
function Base.convert(::Type{ROCPtr{T}}, managed::Managed{M}) where {T, M}
    # let null pointers pass through as-is
    ptr = convert(ROCPtr{T}, managed.mem)
    if ptr == ROC_NULL
        return ptr
    end
    ...
    if M == DeviceMemory && state.context != managed.mem.ctx
        ...
        if maybe_enable_peer_access(state.device, source_device) != 1
            throw(
                ArgumentError(
                    ...
                    by calling `AMDGPU.device!($(deviceid(source_device)))`, or copy the data to an array allocated on device $(deviceid(state.device))."""
                )
            )
        end
        ...
        if stream_ordered(source_device)
            ...
            HIP.access!(pool, state.device, HIP.hipMemAccessFlagsProtReadWrite)
        end
    end
...
@inline function _pool_free(mem::DeviceMemory, stream::HIPStream)
    ...
    return account!(memory_stats(mem.dev), -sizeof(mem))
end
@inline _pool_free(mem::UnifiedMemory, stream::HIPStream) = free(mem)
@inline _pool_free(mem::HostMemory, stream::HIPStream) = free(mem)

"""
	reclaim([sz=typemax(Int)])

Reclaims `sz` bytes of cached memory. Use this to free GPU memory before calling into
functionality that does not use the AMDGPU memory pool. Returns the number of bytes
actually reclaimed.
"""
function reclaim(sz::Int = typemax(Int))
    ...
    return if stream_ordered(dev)
        device_synchronize()
        synchronize(context())
        HIP.trim(pool_create(dev))
    else
        0
    end
end
...
Returns the amount of memory from the AMDGPU memory pool that is currently in use by the
application.
"""
function used_memory()
    state = active_state()
    return if stream_ordered(state.device)
        pool = pool_create(state.device)
        Int(HIP.attribute(UInt64, pool, HIP.hipMemPoolAttrUsedMemCurrent))
    else
        missing
    end
end
...
Returns the amount of backing memory currently allocated for the AMDGPU memory pool.
"""
function cached_memory()
    state = active_state()
    return if stream_ordered(state.device)
        pool = pool_create(state.device)
        Int(HIP.attribute(UInt64, pool, HIP.hipMemPoolAttrReservedMemCurrent))
    else
        missing
    end
end
